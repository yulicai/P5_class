
class Weirdo {
    constructor(x, y, weight, breathTime) {
        this.x = x;
        this.y = y;
        this.weight = weight;
        this.breathTime = breathTime;
        this.mouthSize = createVector(10,20);
    }

    display() {
        //脸
        noFill();
        stroke(255,200);
        strokeWeight(2);
        ellipse(this.x, this.y, 2 * this.weight);
      //眼睛
      let eyeLenght=random(5,10);
      strokeWeight(3);
    	line(this.x-this.weight,this.y-this.weight/2,this.x-this.weight+eyeLenght,this.y-this.weight/2);
      line(this.x,this.y-this.weight/2,this.x+eyeLenght,this.y-this.weight/2+3);  
      
      //嘴
        push()
        let mouth_pos = createVector(this.x , this.y + this.weight / 2);
        imageMode(CENTER); //用push 和 pop限制 只对嘴巴这张图片作用
        image(mouth, mouth_pos.x, mouth_pos.y, this.mouthSize.x, this.mouthSize.y);
        // image(mouth, mouth_pos.x, mouth_pos.y, 10,20);
        pop();
    }

    update() {
        let increment = TWO_PI / this.breathTime;
        let a = this.weight/2;
        let basicSize = 20;
        // 用sin方式让嘴巴大小和声音音量大小有变化
        let scale = a * sin(increment * counter) +a+basicSize;
        this.mouthSize = createVector(scale,scale);
    }
}
